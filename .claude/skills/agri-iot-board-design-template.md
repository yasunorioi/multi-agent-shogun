---
description: >
  Generates custom PCB design templates for agricultural IoT nodes using KiCad/JLCPCB.
  Covers Grove Shield alternatives, relay circuits, I2C bus extension (P82B96), and LCSC part selection.
  Use when: "PCB design", "基板設計", "board design", "Grove Shield", "JLCPCB",
  "IoT node PCB", "カスタム基板", "relay circuit design", "I2Cバッファ基板".
  Do NOT use for: firmware generation, software driver code, or non-PCB hardware topics.
argument-hint: "<configuration: full|minimal> [--sensors sht40,bmp280] [--actuators relay,solenoid]"
---
...
| 製品 | 用途 | 特徴 |
|------|------|------|
| **センサーノード** | 温湿度/CO2計測 | Grove I2C + I2Cハブ + 5Vファンコネクタ |
| **リレーノード** | 電磁弁/ポンプ等 | 既製品活用（Waveshare Pico-Relay-B等） |
...
自作せず、既製品を活用する。GPIO→ターミナルブロック出しの汎用製品。
