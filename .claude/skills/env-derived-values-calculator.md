---
description: >
  Generates a module that calculates agricultural derived values from temperature/humidity:
  saturation deficit (飽差), dew point (露点), absolute humidity, VPD.
  CircuitPython/MicroPython compatible (math module only).
  Use when: "飽差計算", "saturation deficit", "VPD", "露点計算", "dew point",
  "絶対湿度", "absolute humidity", "環境制御計算", "derived values".
  Do NOT use for: sensor reading/driver code or full environmental control logic.
argument-hint: "[python|javascript|c] [module|functions|class]"
---
