---
description: >
  Generates agricultural Home Assistant configuration templates: MQTT sensors, solenoid valve control,
  automation rules, and dashboards for greenhouse/house cultivation.
  Use when: "Home Assistant 農業", "HA agri", "温室制御 HA", "greenhouse HA",
  "HA設定", "灌水自動化", "irrigation automation HA", "farm dashboard".
  Do NOT use for: non-agricultural HA setups or HA OS installation/networking.
argument-hint: "<project_name e.g. my_farm> [--houses 2] [--sensors temp,humidity,co2]"
---
# Home Assistant 農業向け初期設定スキル

> 統合済み: ha-integration-designer の内容を含む
...
## 統合元からの補足: ha-integration-designer

### 汎用HA統合設計パターン（農業以外のHA統合にも適用可能）

#### 統合方式の比較フレームワーク

外部デバイス（Modbus機器、RS485機器、REST APIデバイス等）をHome Assistantに統合する際の意思決定フレームワーク。

| 方式 | リアルタイム性 | 設定容易さ | 安定性 | 適用場面 |
|------|--------------|-----------|--------|---------|
| 公式Integration | ◎ | ◎ | ◎ | 対応デバイスがあれば最優先 |
| MQTT経由 | ◎ | ○ | ○ | カスタムプロトコル、複数システム共有 |
| REST経由 | △ | ○ | ○ | ポーリング型、WebAPI提供デバイス |
| Node-RED経由 | ◎ | ○ | ○ | 複雑ロジック、データ変換必要時 |
| Modbus直接 | ○ | △ | ◎ | 産業機器、レジスタ仕様明確な場合 |

#### Modbus統合の詳細設計例

```yaml
# configuration.yaml - Modbus RTU デバイス統合
modbus:
  - name: device_controllers
    type: serial
    port: /dev/ttyUSB0
    baudrate: 9600
    bytesize: 8
...
**問題1: センサーが "unavailable"**
- 原因候補: シリアルポート誤り、ボーレート不一致、アドレス誤り、配線反転
- 確認: `mbpoll -a 1 -b 9600 -P none -t 4 -r 100 /dev/ttyUSB0`

**問題2: 値が異常（桁違い）**
- 原因候補: scale設定誤り、data_type誤り、レジスタアドレス誤り
- 確認: デバイスマニュアルでレジスタ仕様確認、生値をmbpollで確認

**問題3: 書き込みができない**
- 原因候補: デバイスが書き込み禁止モード、Function Code不一致
- 確認: `mbpoll -a 1 -b 9600 -P none -t 4 -r 101 /dev/ttyUSB0 280`

#### 統合アーキテクチャパターン

**パターン1: 公式Integration優先**
```
...
1. **シリアルポート確認**: `ls -la /dev/ttyUSB*`
2. **Modbus通信テスト**: `mbpoll` コマンドでレジスタ読み取り
3. **HA設定反映**: `ha core check` → `ha core restart`
4. **エンティティ確認**: Developer Tools → States で値確認
5. **アラートテスト**: 閾値超過で通知受信確認
...
- alias: "High Temperature Alert"
  ...
  action:
    - service: notify.mobile_app
      data:
        title: "高温警報"
        message: "温度: {{ states('sensor.controller_1_temperature') }}℃"

# 通信エラーアラート（2分継続で発火）
- alias: "Communication Error"
  trigger:
    - platform: state
      entity_id: sensor.controller_1_temperature
      to: "unavailable"
  ...
  action:
    - service: notify.mobile_app
      data:
        title: "通信エラー"
        message: "デバイスとの通信が途絶しました"
```

#### ログ確認とデバッグ

```yaml
# configuration.yaml - 詳細ログ有効化
logger:
  default: info
  logs:
    pymodbus: debug
...
この補足により、農業以外の産業機器（工場、BEMS、スマートホーム）のHA統合にも対応可能となります。
