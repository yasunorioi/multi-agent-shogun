---
description: >
  Generates SQLite/PostgreSQL database schemas for IoT timer/scheduler systems.
  Includes sunrise/sunset linking, 8-timeslot control, sequence control, and execution logs.
  Use when: "timer DB", "タイマーDB", "scheduler database", "スケジューラDB",
  "日出連動タイマー", "sunrise timer", "灌水シーケンスDB", "irrigation sequence DB".
  Do NOT use for: Node-RED flow generation (use nodered-timer-flow-generator) or non-timer DB schemas.
argument-hint: "<sqlite|postgres> [--features sunrise,sequence,8slot]"
---
