...
1. **自分のIDを確認**: `tmux display-message -t "$TMUX_PANE" -p '#{@agent_id}'`
   - `shogun`→将軍 / `karo-roju`→老中 / `ashigaru1`→足軽1 / `ashigaru6`→部屋子1 / `ohariko`→お針子
2. **対応する instructions を読む**（上記の対応表参照）
...
```
上様（人間）→ SHOGUN（将軍）→ ROJU（老中）→ 足軽/部屋子
                               ↑ OHARIKO（お針子：監査・先行割当）
```
...
- **報告の流れ**: 足軽→老中(send-keys可) / 老中→将軍(dashboard.mdのみ、send-keys禁止) / お針子→老中(send-keys可)
...
| 部屋子1(ashigaru6) | multiagent:agents.2 | multiagent |
| お針子 | ooku:agents.0 | ooku |
| 高札 | ooku:agents.1 | ooku (Docker, API: http://localhost:8080) |
...
queue/inbox/{karo}_reports.yaml   # 報告inbox（足軽→家老）
