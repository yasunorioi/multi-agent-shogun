...
| 高札 | ooku:agents.2 | ooku (DEPRECATED: CLI統合済み。python3 scripts/botsunichiroku.py search) |
...
scripts/botsunichiroku_2ch.py     # 2ch DAT表示レイヤー（cmd_419追加）
scripts/migrate_fts5.py           # FTS5マイグレーション（初回のみ）
...
作業開始前に、利用可能なスキル（`/` コマンド）の中に **現在のタスクに該当するものがないか** を必ず確認せよ。

### ルール

1. **1%でも該当する可能性があればスキルを発火せよ。** スキップは禁止。
2. スキルの `description` はトリガー条件を記載している。内容の要約ではない。
3. スキル本体は `Skill` ツールでオンデマンド読み込み。事前に全文を暗記する必要はない。
4. 「このタスクは単純だからスキル不要」は **よくある言い訳** である。単純なタスクこそスキルで品質を担保せよ。

### よくある言い訳テーブル（スキル発火回避の禁止）

| 言い訳 | 正しい行動 |
|--------|-----------|
| 「単純だからスキル不要」 | 単純でもスキルを確認。該当すれば発火 |
| 「スキルを読む時間がもったいない」 | スキルを読まずにミスする方が高コスト |
| 「前にやったから覚えている」 | 記憶に頼るな。毎回スキルを読め |
| 「今回だけ例外」 | 例外はない。ルールに従え |
