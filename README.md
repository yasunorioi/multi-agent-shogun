...
お針子は `needs_audit=1` のsubtask完了時に自動起動するが、成果物がコード/文書でないタスクでは監査対象がなく、長期間アイドルになる。

- 空振りの例: 実機FW転送、SDカードイメージビルド、OTAテスト、Node-RED UI手動設定、HWデバッグ
- 対策: 実機作業が続く期間は、将軍/老中がお針子にコードベース全体監査やDB整合性チェックを手動で振ること
