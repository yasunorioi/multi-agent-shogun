...
> **実施日**: 2026-03-21 | **担当**: ashigaru1 (subtask_946 / cmd_425)

### 7-1. 前提確認

| 項目 | 結果 | 判定 |
|------|------|------|
| `claude --version` | **2.1.80** | ✅ AT要件(v2.1.32+)クリア |
| `claude --version` | **2.1.80** | ✅ Channels要件(v2.1.80+)クリア |
| `~/.claude/teams/` | **存在しない**（新規作成） | AT未使用を確認 |
| `~/.claude/tasks/` | **存在する**（55ディレクトリ、全て空） | 過去セッションの残骸 |

### 7-2. settings.json 設定追加
...
**注**: `teammateMode` はsettings.jsonの公式スキーマに**存在しない**。
CLIフラグ `claude --teammate-mode in-process` で指定するか、環境変数で対応。
...
```
~/.claude/teams/phase0-test/config.json   # チーム設定
~/.claude/tasks/phase0-test/
  task-001.json  # 独立タスク（self-claim対象）
  task-002.json  # task-001依存タスク（ブロック確認用）
  task-003.json  # 独立タスク（並列self-claim確認用）
```
...
```json
{
  ...
  "assignedTo": null,          // self-claimするteammateのagentId
  "dependencies": [],          // 依存taskのidリスト
  "createdAt": "..."
}
```

**self-claimのロジック（推定）**:
1. Teammateが完了 → 次の `status: pending` かつ `dependencies` が全て `completed` のタスクを取得
2. `assignedTo` に自分のagentIdを設定（ファイルロックで競合防止）
3. `status: in_progress` に変更して作業開始

### 7-4. tmux干渉リスク実測

**現在のshogun環境**:
```
multiagent:0 → 4ペイン (agents.0-3) — 全て稼働中
ooku:0       → 4ペイン (agents.0-3) — 全て稼働中
shogun:0     → 1ペイン (main)       — 稼働中
合計: 9ペイン
```

**ATがsplit-panesモードで起動した場合のリスク**:

| リスク | 詳細 | 深刻度 |
|--------|------|--------|
| **ペイン追加** | ATが現在のtmuxセッションにペインを追加 | 🔴 高 |
| **ペイン番号ズレ** | multiagent:0.4+ が追加され、既存scripts（agents.0-3前提）が壊れる | 🔴 高 |
| **@agent_id混乱** | shutsujin_departure.shが設定した@agent_idとAT管理のagentIdが衝突 | 🟡 中 |
| **paneタイトル上書き** | ATがペインタイトルを変更し、stop_hookの判定に影響 | 🟡 中 |

**⚠️ 結論: shogun既存セッション内でAT split-panesモードは危険**

### 7-5. 安全な実行方法（Phase 0推奨手順）

```bash
# 方法A: in-processモードで実行（推奨）
# 既存shogunセッションと分離。新ターミナルまたは別ウィンドウから:
CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1 \
  claude --teammate-mode in-process --project /home/yasu/multi-agent-shogun

# 方法B: 別tmuxセッションで実行（split-panesを試したい場合）
tmux new-session -s at-test -d
tmux send-keys -t at-test "CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1 claude" Enter
# → at-testセッション内でATがsplit-panesを生成（shogunと分離）

# ❌ 禁止: multiagent/ooku/shogunセッション内でAT起動
# → 既存ペイン配置が破壊される
```

### 7-6. self-claim動作確認（次ステップ）

実際のself-claim動作確認は別セッションで実施が必要。確認手順:
...
# 2. Leadセッションで指示
"Create a team with 2 teammates and have them work through the tasks
 in ~/.claude/tasks/phase0-test/ using self-claim."

# 3. 確認項目
# - task-001, task-003が並列でself-claimされるか
# - task-001完了後にtask-002がアンブロックされるか
# - ステータスラグ（完了マークの遅延）が発生するか
# - トークン消費量（現行shogunと比較）
```

**注**: 本subtask(946)では環境構築と手順整備まで実施。実際の動作確認は次のPhase 0試行セッションで実施すること。
