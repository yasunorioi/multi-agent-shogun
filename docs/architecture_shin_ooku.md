...
LLMのコンテキスト消失（コンパクティング）問題に対する解決策として、**外部DB（没日録）** と **検索サービス（鯰）**、そして **監査役（お針子）** を導入しています。
...
### 2.5 鯰 (Namazu) - Intelligence Service
*   **役割**: 検索・分析・可観測性サービス（Search & Observability）。
*   **ツール**: `tools/botsunichiroku-search` (FastAPI + SQLite FTS5)
*   **機能**:
    *   過去の似たタスクを検索（RAG）。
...
graph TD
    ...
    subgraph "Oocu / Audit Layer"
        Namazu[鯰 (Search Service)] -.-> DB
        Ohariko -- "監査/孤児検出" --> Namazu
