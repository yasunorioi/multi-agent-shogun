...
LINE Platform → Webhook → VPS (HTTPS受信)
  → VPS内 WG経由で MBP:5000 に転送（リバースプロキシ的役割）
  → MBP LINE Bot がメッセージ処理
```

VPSは既存のWebhookリレー機能をそのまま利用。新規月額コストなし。

### 分離の理由

- 殿の管理用WG（10.10.0.x）と農家用WG（10.20.0.x）を同一ネットワークにすると、農家RPiから殿のネットワークにアクセス可能になる
- CIDRが異なるため**ルーティングレベルで完全分離**
- MBPは両方のWGインターフェースを持つが、IP転送（ip_forward）は農家→MBP方向のみ許可
...
wg_server_endpoint: <VPS_IP>:51821
wg_client_ip: 10.20.0.10/32
wg_client_private_key: <FARMER_A_PRIVATE_KEY>
api_endpoint: http://10.20.0.1:5000
```

RPi側はこれをデコードして `wg-farmers-client.conf` を自動生成し、WGを起動する。
...
with tab_settings:
    ...
    if st.button("登録"):
        try:
            ...
            # WG設定ファイル生成
            wg_conf = f"""[Interface]
PrivateKey = {config['wg_client_private_key']}
...
- Webhook受信 → WG経由でMBP:5000に転送（nginx reverse proxy）
- VPSのnginx設定例:
...
server {
    ...
    location /webhook {
        proxy_pass http://10.20.0.1:5000/webhook;  # MBPのWG IP
...
farmers:
  farmer_a:
    ...
    wg_public_key: "aBcDeFgHiJkLmNoPqRsTuVwXyZ="
    wg_ip: "10.20.0.10"
    wg_client_private_key_b64: "<Base64エンコード済み秘密鍵>"
...
| 農家RPi WGクライアント秘密鍵 | RPi `/etc/wireguard/wg-farmers-client.conf` | RPiローカルのみ |
| 農家RPi WGクライアント秘密鍵（バックアップ） | MBP `farmers_secrets.yaml` （Base64） | オンボーディング時に生成。RPi再セットアップ用 |
