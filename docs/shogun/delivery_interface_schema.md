...
severity: S2                   # FAIL/CONDITIONAL時必須 | enum | S1/S2/S3/S4
...
| `severity` | FAIL/CONDITIONAL時**必須** | enum | `S1`/`S2`/`S3`/`S4` | S1=Critical, S2=Major, S3=Minor, S4=Trivial |
...
| `FAIL S1` | Critical。設計根本ミス | リジェクト+殿エスカレーション |
| `FAIL S2` | Major。機能不全 | リジェクト+RAG検索+フル再検収 |
| `FAIL S3` | Minor。品質基準未達 | リジェクト+老中スポットチェックで再検収 |
