...
| Ethernet | W5500 **SPI0**接続 (GPIO33-36), 10/100Mbps RJ45 |
...
### I2C1 (RTC+センサー用) ※GPIO6/7はRP2350 pinmux上I2C1
...
| 5 | RS485 RX (UART1) | IN |
| 6 | I2C1 SDA (RTC+SHT40) | I/O |
| 7 | I2C1 SCL (RTC+SHT40) | OUT |
| 8 | (未割当) | — |
| 9-16 | DI1-DI8 | IN |
| 17-24 | Relay 1-8 | OUT |
| 25 | W5500 RST | OUT |
...
### 8.6 USB-UARTデバッグ出力（殿裁定 2026-04-08）

FW書き込み後にIPアドレスが行方不明になる問題への対策。
UART0 (GPIO0/1) = USB-C経由のSerial出力を常時デバッグチャネルとして使用。

| 出力タイミング | 内容 | 書式例 |
|-------------|------|--------|
| 起動時 | ホスト名、MAC、DHCP取得IP、ゲートウェイ、サブネット | `[BOOT] hostname: ccm-rp2350.local` |
| DHCP取得時 | IPアドレス確定通知 | `[NET] ip: 192.168.1.100` |
| 30秒毎 | IP、mDNS状態、MQTT接続状態、uptime | `[STATUS] ip:192.168.1.100 mqtt:OK up:3600s` |
| エラー時 | DHCP失敗、MQTT切断、W5500初期化失敗等 | `[ERR] DHCP timeout, retrying...` |

```cpp
// setup() 内
Serial.begin(115200);  // UART0 = USB Serial
...
if (millis() - last_status > 30000) {
  Serial.printf("[STATUS] ip:%s mqtt:%s up:%lus\n",
    ip, mqtt_connected ? "OK" : "DISC", millis()/1000);
  last_status = millis();
}
```

USB-Cケーブル接続 + `screen /dev/ttyACM0 115200` で即座にIP確認可能。
別途USB-UARTアダプタは不要（UART0がUSB-C経由で出る）。
