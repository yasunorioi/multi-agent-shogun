...
**PDCAフロー:**
1. **Plan**: 軍師が設計・分析を完了し、gunshi_analysis.yamlに記載
2. **Do**: 家老が足軽に実装を割り当て、足軽がパイロット実装
3. **Check**: お針子が監査、軍師のpredicted_outcomeと実結果を突合
4. **Act**: ズレがあれば軍師に差分分析依頼→修正指示

- 最大3イテレーション。3回で収束しなければ家老にエスカレーション
- 各イテレーションの結果をgunshi_analysis.yamlに追記（iteration_log）
