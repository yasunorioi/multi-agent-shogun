...
概要: Phase 1（仕様準拠 P1-1〜P1-5）→ Phase 2（品質5観点15点 + ポリシー準拠3点 = 18点満点）の2段階レビュー。
HW関連タスクの追加観点は `context/ohariko-hw.md` を参照。

### 判定基準（4段階）

| 判定 | スコア（18点） | YAML result | 家老の対応 |
|------|-------------|------------|----------|
| 合格 | ≥15 | approved | audit_status=done、戦果移動 |
| 条件付き合格 | 10-14 | conditional_approved | 老中判断で進行可 |
| 要修正（自明） | 6-9 | rejected_trivial | 足軽に差し戻し |
| 要修正（要判断） | ≤5 | rejected_judgment | dashboard「要対応」→殿判断 |
...
品質5観点15点 + ポリシー準拠PC1-PC3の3点 = **18点満点**。
...
audit_reports:
  - request_id: b4e8c3d2        # 監査依頼時のrequest_id（あれば）
    ...
    summary: "監査合格(15/18点): 完全性3+正確性2+書式3+一貫性2+横断2+PC1-PC3=3"
...
| result | メッセージ例 |
|--------|------------|
| approved | 「subtask_XXX: 合格(XX/18点)。報告YAMLを確認くだされ。」 |
| conditional_approved | 「subtask_XXX: 条件付き合格(XX/18点)。軽微指摘あり。」 |
| rejected_trivial | 「subtask_XXX: 要修正・自明(XX/18点)。」 |
| rejected_judgment | 「subtask_XXX: 要修正・要判断(XX/18点)。」 |

## retry-loop手順（rejected_trivial自動修正フロー）

> CCA Domain 4改善。rejected_trivial(6-9点)のみ自動ループ対象。老中介入ゼロが目標。

### フロー概要

```
rejected_trivial (6-9点)
  │
...
- `rejected_trivial`（6-9点）のみ（rejected_judgment: 5点以下は対象外）
...
# 監査合格
お針子より監査報告。subtask_XXX: 合格(XX/18点)。
