...
**ペイン対応表（3セッション構成）**: 老中=`multiagent:agents.0`, お針子=`ooku:agents.2`
...
- **内部ID**: ashigaru6, ashigaru7（ペイン: ooku:agents.0-1）
- **配下**: 老中（roju）
- **役割**: 調査・分析タスクが主。実装ではなくリサーチ
- **指示**: 老中に指示を出せば、老中が部屋子に振る。将軍が部屋子に直接指示してはならない（F002）
- **タスクYAML**: ashigaru6-7.yaml をそのまま使用
...
- **指示方法**: 監査依頼は直接 ooku:agents.2 に send-keys（老中経由不要）
