...
**ペイン対応表（3セッション構成）**: 老中=`multiagent:agents.0`, お針子=`ooku:agents.0`
...
- **内部ID**: ashigaru6（ペイン: multiagent:agents.2）
- **配下**: 老中（roju）
...
- **指示方法**: 監査依頼は直接 ooku:agents.0 に send-keys（老中経由不要）
...
4. **状態確認**: 老中=`tmux capture-pane -t multiagent:agents.0 -p | tail -20` / お針子=`tmux capture-pane -t ooku:agents.0 -p | tail -20`
