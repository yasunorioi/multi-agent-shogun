...
cmd_status() {
    ...
    for agent_id in gunshi ashigaru1 ashigaru2 ashigaru6 ohariko; do
        local pane="${PANE_MAP[$agent_id]}"
        local state
        state=$(get_pane_state "$pane")
...
cmd_count_needed() {
    ...
    local running=0
    for agent_id in gunshi ashigaru1 ashigaru2 ashigaru6; do
        local state
...
show_usage() {
    ...
    echo ""
    echo "Agent IDs: gunshi (軍師), ashigaru1 (足軽1), ashigaru2 (足軽2), ashigaru6 (heyago1), ohariko"
    echo ""
    echo "Examples:"
