...
# ═══════════════════════════════════════════════════════════════════════════════
# STEP 5.2: ooku セッション作成（十字4ペイン：軍師 + お針子 + 高札 + 獏(baku)）
# ═══════════════════════════════════════════════════════════════════════════════
...
# ooku ペイン設定
OOKU_LABELS=("gunshi" "ohariko" "kousatsu" "baku")
OOKU_COLORS=("magenta" "yellow" "green" "white")
OOKU_AGENT_IDS=("gunshi" "ohariko" "kousatsu" "baku")

if [ "$KESSEN_MODE" = true ]; then
    OOKU_TITLES=("gunshi(Opus)" "ohariko(Opus)" "kousatsu(高札API)" "baku(獏)")
    OOKU_MODELS=("Opus Thinking" "Opus Thinking" "FTS5+MeCab" "baku")
else
    OOKU_TITLES=("gunshi(Opus)" "ohariko(Sonnet)" "kousatsu(高札API)" "baku(獏)")
    OOKU_MODELS=("Opus Thinking" "Sonnet Thinking" "FTS5+MeCab" "baku")
fi
...
if [ "$SETUP_ONLY" = false ]; then
    ...
    if [ "$IDLE_MODE" = true ]; then
        ...
        # 獏(baku)は省力時も起動
        p=$((PANE_BASE + 3))
        tmux send-keys -t "ooku:agents.${p}" "python3 scripts/baku.py"
        ...
        # ═══════════════════════════════════════════════════════════════════════════
        # ooku セッション: 軍師 (pane 0) + お針子 (pane 1) + 獏 (pane 3)
        # ═══════════════════════════════════════════════════════════════════════════
        ...
        # 獏(baku) (pane 3): LLM使用量監視（Claude Code不要）
        p=$((PANE_BASE + 3))
        tmux send-keys -t "ooku:agents.${p}" "python3 scripts/baku.py"
        tmux send-keys -t "ooku:agents.${p}" Enter
        log_info "  └─ 獏（LLM使用量監視）、起動完了"
    fi
...
echo "     │ pane 0 - Opus      │ pane 1 - Sonnet    │"
echo "     ├────────────────────┼────────────────────┤"
echo "     │ kousatsu (高札)    │ baku (獏)          │"
echo "     │ pane 2 - Docker    │ pane 3 - baku.py   │"
echo "     └────────────────────┴────────────────────┘"
echo ""
